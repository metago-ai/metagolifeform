...
| 参数值 | 平台 | 规则文件 | 默认注入路径 |
|--------|------|----------|-------------|
| `trae` | Trae（默认） | `rules.md` | `~/.trae-cn/rules.md` |
...
Copy-Item ".\rules.md" "$env:USERPROFILE\.trae-cn\rules.md" -Force
...
```powershell
# Trae
Test-Path "$env:USERPROFILE\.trae-cn\rules.md"
