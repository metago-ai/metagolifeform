...
> 一次安装，拥有完整思维工具箱。不是 39 个独立插件，而是一个有机整体。

## 技能族谱

MetaGO 的 39 个技能分为 11 个能力族，每个族解决一类核心问题：
...
| 工程质量族 | 2 | 交付前原子验证 + AI 自律执行 | metago-delivery-gate |
| **合计** | **39** | | |
...
| 技能 | 功能 | 触发场景 |
|------|------|----------|
| [metago-delivery-gate](./delivery-gate) | 交付前原子验证门控 | 三层验证（技术/业务/链路）+ 任何一项 FAIL 禁止宣告完成 |
| [metago-discipline](./discipline) | AI 自律执行协议 | 五问自检：是否运行 verify / 是否有 FAIL / 报告含验证小节 / 每项 ✅ 有证据 / 做了业务层验证 |

## 与 MCP tools 的关系

39 技能中，22 个被封装为 MCP tools（结构化参数），加上 20 个独有思维工具，7 个同名工具在 MCP 中合并（TOOLKIT_TOOLS 优先），加上 2 个工程质量族工具，共 53 tools（公式：53 tools = 39 skills + 14 toolkit (Engine V2 硬驱动)）。
...
- [8 条公理 + 7 条属性](../engine/axioms) — 理解技能背后的法则
- [MCP Server 53 tools](../api/mcp-server) — 通过 MCP 协议调用
