...
[将 39 个技能的 name 与 description 列表粘贴于此]
```
...
│   ├── skills-data.ts    39 个技能文件夹（37 个导出为元数据，tools 源）
│   └── prompts.ts        8 个引导词元数据（prompts 源）
├── package.json          @metago-ai/mcp-server
└── tsconfig.json
```
...
export interface SkillMeta {
  ...
  /** 工具描述（展示给客户端） */
  description: string;
  /** 能力族：认知族 / 保障族 / 治理族 / 进化族 / 执行族 / 溯源族 / 价值族 / 意识族 / 方法论族 / 架构族 / 工程质量族 */
  category: string;
...
| `category` | 限定为：认知族 / 保障族 / 治理族 / 进化族 / 执行族 / 溯源族 / 价值族 / 意识族 / 方法论族 / 架构族 / 工程质量族；自定义技能建议归入"保障族"或新建族后同步更新 `category` |
...
启动后控制台会输出 `[@metago-ai/mcp-server] 服务已启动，等待客户端连接...`，随后即可在 MCP 客户端配置中通过 `npx -y @metago-ai/mcp-server` 或本地路径 `node ./packages/mcp-server/dist/index.js` 调用。
...
5. **能力族归类**：自定义 tool 的 `category` 须归入既有 11 大能力族，便于客户端按族筛选。
