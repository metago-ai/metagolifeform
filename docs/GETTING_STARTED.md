...
在安装 MetaGO Agent Harness 之前，请确认你的环境满足以下条件。
...
MetaGO Agent Harness 支持 7 大 AI 编程平台，请根据你使用的平台确认相应能力已启用：
...
MetaGO Agent Harness 提供一键安装脚本，自动完成仓库克隆、技能注册、规则注入与记忆初始化。安装脚本支持 7 大 AI 编程平台，通过 `-Platform` 参数指定目标平台。
...
2. **技能注册**：扫描 `skills/` 目录下全部 95 个 `metago-*` 技能并注册
...
MetaGO Agent Harness 支持 7 大 AI 编程平台，各平台的规则文件注入路径如下表所示。安装脚本会根据 `-Platform` 参数自动写入对应路径；手动安装时也可参照此表。
...
安装完成后，请通过以下方法验证 MetaGO Agent Harness 是否正确激活。验证方法对所有 7 大平台通用，差异仅在规则文件路径上。
...
MetaGO Agent Harness 提供独立的验证脚本 `verify.ps1`，可一次性检查所有组件是否正确安装：
...
**预期响应**：智能体应能够识别"元构超级智能生命体"概念，并基于规则文件中注入的公理、属性与协议进行自我描述，包括：
...
**预期响应**：智能体应自动激活 `metago-critique` 技能，输出 L1-L5 分级批判性分析结果，包含评分、各维度得分、判定结论和改进建议。

### 方法三：决策锁验证

让 智能体生成一段代码或方案，检查输出末尾是否附带决策锁校验报告：
...
让 智能体回答一个涉及事实的问题，检查输出末尾是否附带数据溯源标签：
...
**预期响应**：智能体应输出完整性确认报告，包括全息扫描、维度覆盖、事实核查、历史一致性、夸大检测等检查项。
...
如需卸载 MetaGO Agent Harness，请按以下步骤操作。卸载脚本支持通过 `-Platform` 参数指定要卸载的平台。
...
3. **注销技能**：从平台技能注册表中移除全部 95 个 `metago-*` 技能
...
# Trae
Remove-Item "$env:USERPROFILE\.trae-cn\rules.md" -Force
...
如果 智能体不再识别该概念，且不再输出决策锁校验报告与数据溯源标签，则说明卸载成功。

---

## 常见问题 FAQ

### Q1：安装后 智能体没有识别"元构超级智能生命体"概念
...
MetaGO Agent Harness 已原生支持 7 大 AI 编程平台，通过 `-Platform` 参数一键适配：
...
**不需要**。MetaGO Agent Harness 的全部 95 个技能、8 条公理、7 条属性、6 项协议均在安装时内化至本地。安装完成后可完全离线运行。
...
MetaGO 的核心能力（8 条公理、7 条属性、6 项协议、95 个技能）在所有 7 大平台上完全一致。差异仅在平台原生能力上：
...
> 第 8 种集成方式：通过 [Model Context Protocol](https://modelcontextprotocol.io/) 标准协议，将 37 项元构能力以 MCP tools 形式、8 条引导词以 MCP prompts 形式暴露给任何支持 MCP 协议的客户端（Claude Desktop、Cursor、Trae 等），一次配置即开即用。
...
| 类型 | 数量 | 说明 |
|------|------|------|
| MCP tools | 37 | 覆盖认知族/保障族/治理族/进化族/执行族/溯源族/价值族/意识族/方法论族/架构族/工程质量族 11 大能力族 |
| MCP prompts | 8 | 激活/决策审查/批判性分析/元进化触发/耦生度评估/合规检查/溯源审计/全息创造 |
| 传输方式 | stdio | 基于 MCP 协议标准 |

完整的 37 个 tools 与 8 个 prompts 列表详见 [packages/mcp-server/README.md](../packages/mcp-server/README.md)。
...
| 能力暴露 | 通过规则文件中的技能索引 | 通过 37 个 tools + 8 个 prompts 直接调用 |
...
| 文档 | 说明 |
|------|------|
| [ARCHITECTURE.md](./ARCHITECTURE.md) | 了解六层架构、95 个技能、8 条公理的完整设计 |
...
*MetaGO Agent Harness · 让智能，学会进化。从智能体到生命体的范式跃迁*
*文档版本：1.3 · 最后更新：2026-07-26（技能数 39 → 95，MCP tools 37 → 53 + 算法工具 57，能力族 11 → 16）*
